...
export default function DashboardPage() {
  ...
  const stats = [
    { label: 'Total Matches', value: analytics?.totalDebates || 0, icon: Crosshair, color: 'text-brand-400', shadow: 'shadow-neon-blue' },
    { label: 'Win Rate', value: `${analytics?.winRate || 0}%`, icon: Target, color: 'text-emerald-400', shadow: 'shadow-[0_0_15px_rgba(52,211,153,0.3)]' },
    { label: 'Hot Streak', value: analytics?.currentStreak || 0, icon: Zap, color: 'text-rose-500', shadow: 'shadow-[0_0_15px_rgba(244,63,94,0.3)]' },
    { label: 'Global Rank', value: analytics?.rank || 'Unranked', icon: Trophy, color: 'text-rank-gold', shadow: 'shadow-rank-gold' },
  ];
            ...
            <div className={`glass-panel p-6 h-full relative overflow-hidden group hover:${stat.shadow} transition-shadow duration-300`}>
              ...
              <div className="flex items-start justify-between relative z-10">
                <div>
                  ...
                  <p className="text-3xl sm:text-4xl font-heading font-bold text-white mt-2 drop-shadow-md">{stat.value}</p>
                </div>
